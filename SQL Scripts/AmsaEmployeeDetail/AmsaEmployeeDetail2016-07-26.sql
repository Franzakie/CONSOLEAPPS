  ...
  SELECT COUNT(*) INTO TmpRecExist FROM AMSA_EMPLOYEE_DETAIL WHERE PERNO = v_PERNO;
    IF TmpRecExist = 0 THEN
        BEGIN
            ...
            INTO TmpNewId
            FROM Dual;
            INSERT INTO AMSA_EMPLOYEE_DETAIL ("ID", PERNO, "NAME", SURNAME, NATIONALID, EMAIL, ORGUNIT, OBJABREV, OBJNAME, INITIALS, PERSONALAREA, PERSONALAREADESC, PERSONALSUBAREA, PERSONALSUBAREADESC)
            VALUES (TmpNewId, v_PERNO, v_NAME, v_SURNAME, v_NATIONALID, v_EMAIL, v_ORGUNIT, v_OBJABREV, v_OBJNAME, v_INITIALS, v_PERSONALAREA, v_PERSONALAREADESC, v_PERSONALSUBAREA, v_PERSONALSUBAREADESC);
        END;
    ELSE
