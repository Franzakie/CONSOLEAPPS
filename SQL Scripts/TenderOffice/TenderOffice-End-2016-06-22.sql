...
BEGIN
    ...
    Str_Value4 := '';
    v_ID := 0;
	SELECT COUNT(*) INTO myCount FROM DTREECORE WHERE NAME = (SELECT CONCAT(CONCAT(CONCAT(CONCAT(RFQ_GROUPNO, '-'), RFQNO), '-'), VENDORNAME) FROM AMSA_RFQ WHERE RFQNO = v_DOCID) AND OWNERID=-2000;
  IF myCount > 0 THEN
    BEGIN
      SELECT DATAID INTO v_ID FROM DTREECORE WHERE NAME = (SELECT CONCAT(CONCAT(CONCAT(CONCAT(RFQ_GROUPNO, '-'), RFQNO), '-'), VENDORNAME) FROM AMSA_RFQ WHERE RFQNO = v_DOCID)  AND OWNERID=-2000; 
      IF v_ID > 0 THEN
        Str_AttribNo := '0';
...
BEGIN
    ...
    IF TO_NUMBER(Str_AttribNo) > 0 THEN
        SELECT DATAID INTO v_ID FROM DTREECORE WHERE NAME = v_DOCID AND OWNERID=-2000;  -- OR SHOULD IT BE DTREE?
        SELECT SEGMENTBLOB INTO Str_Value FROM LLATTRBLOBDATA WHERE ID = v_ID AND VERNUM = (SELECT MAX(VERNUM) FROM LLATTRBLOBDATA WHERE ID = v_ID );
