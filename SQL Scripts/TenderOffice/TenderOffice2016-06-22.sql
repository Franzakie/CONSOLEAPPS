	...
	IF v_ID = 0 THEN
		BEGIN
			...
			INSERT INTO AMSA_RFQ (ID, RFQNO, RFQ_GROUPNO, OURREF, DESCRIPTION, CLOSINGDATE, VENDORNO, VENDORNAME, BUYERNO, BUYERNAME, 
					LOCATIONNO, LOCATION)
			SELECT TmpNewId, v_RFQNO, v_RFQ_GROUPNO, v_OURREF, v_DESCRIPTION, TO_DATE(v_CLOSINGDATE, 'yyyymmdd HH24:MI:SS'), v_VENDORNO, v_VENDORNAME, v_BUYERNO, v_BUYERNAME, 
	...
	ELSE
		UPDATE AMSA_RFQ
			SET RFQNO = v_RFQNO, 
				...
				CLOSINGDATE = TO_DATE(v_CLOSINGDATE, 'yyyymmdd HH24:MI:SS'), 
				VENDORNO = v_VENDORNO, 
